...
class TranscriptionRequest(OpenAIBaseModel):
    ...
    # Beam search parameters (custom, not in original OpenAI API)
    use_beam_search: bool = False
    """Whether to use beam search for transcription."""
    
    n: Optional[int] = Field(
        default=None,
        description="(Experimental) Number of beams (beam width) for transcription.")
    beam_width: Optional[int] = Field(
        default=None,
        description="(Experimental) Alias for number of beams (beam width) for transcription.")
    ...
    def to_beam_search_params(
        ...
        # Determine beam width from either beam_width or n parameter
        beam_width = 1
        if self.beam_width is not None and self.beam_width > 1:
            beam_width = self.beam_width
        elif self.n is not None and self.n > 1:
            beam_width = self.n
        ...
        return BeamSearchParams(
            ...
            max_tokens=max_tokens,
            ignore_eos=False,  # For transcription, we typically want to respect EOS
            temperature=temperature,
            length_penalty=1.0,  # Default length penalty for beam search
    ...
    def validate_beam_search_params(cls, data):
        ...
        if use_beam_search or beam_width or (n and n > 1):
            ...
            if beam_width is not None and beam_width > 10:
                raise ValueError("beam_width must be <= 10 for reasonable performance")
            if n is not None and n > 10:
                raise ValueError("n must be <= 10 for reasonable performance")
