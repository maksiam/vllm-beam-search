...
class OpenAIServingTranscription(OpenAIServing):
    ...
    async def create_transcription(
        ...
        try:
            ...
            # === Beam search support: check if beam search is requested ===
            beam_width = None
            if getattr(request, "beam_width", None):
                beam_width = request.beam_width
            elif getattr(request, "n", None) and request.n > 1:
                # Treat 'n' as beam width if >1
                beam_width = request.n
            
            # Set use_beam_search flag if beam width is specified
            if beam_width is not None and beam_width > 1:
                request.use_beam_search = True
            
            # Create appropriate parameters based on beam search setting
        ...
        # Check if streaming is compatible with beam search
        if request.stream and request.use_beam_search:
            return self.create_error_response(
                "Streaming is not supported with beam search. "
                "Set stream=False when using beam search.")
        ...
        try:
            ...
            logger.exception("Unexpected error during transcription processing")
